...
invoice_bp = Blueprint(
    ...
    static_url_path="/invoice/static",
)
...
@invoice_bp.route("/invoice_form/data")
@login_required
...
@invoice_bp.route(
    "/invoice_form/", defaults={"invoice_id": None}, methods=["GET", "POST"]
)
...
def save_invoice(invoice_id):
    ...
    HTML(string=html_out).write_pdf(
        file, stylesheets=[os.path.join(this_folder, "static", "css/styles.css")]
    )
...
def gen_invoice(invoice_id):
    ...
    headers["image_path"] = "file://" + os.path.join(
        this_folder, "static", "logo.png"
    )
